...
public actual inline fun <T> (suspend () -> T).startCoroutineUninterceptedOrReturn(
    ...
    return if (jsTypeOf(d) == "function") d(completion, false)
    else d.invoke(completion)
}
...
public actual inline fun <R, T> (suspend R.() -> T).startCoroutineUninterceptedOrReturn(
    ...
    val d = this.asDynamic()
    return if(jsTypeOf(d) == "function") d(receiver, completion, false)
    else d.invoke_P1(receiver, completion)
}
...
public actual fun <R, T> (suspend R.() -> T).createCoroutineUnintercepted(
    ...
    if (this.asDynamic().length == 3) {
        ...
        createCoroutineFromSuspendFunction(completion) {
            val d = this.asDynamic()
            if (jsTypeOf(d) == "function") {
                d(receiver, completion)
            } else {
                d.invoke_P1(receiver, completion)
            }
        }
    }
